import {
  ...
  getInterestGiftList
} from '@/common/data-source/interest/requestApis';
...
class InterestService {
  ...
  static getPointRecordList() {
      return getUserPointRecordList().then(list=>{
          return list.map((item: pointRecordType) => new PrizeRecord(item))
      })
  }
  ...
  static getInterestGiftList() {
      return getInterestGiftList().then(list => {
          return list.map((item: pointGiftType) => new PointGift(item))
      })
  }
}
